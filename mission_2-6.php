...
if(!empty($_POST["name"]) && !empty($_POST["comment"]) && !empty($_POST["pass"]) && isset($_POST["write"]) && !isset($_POST["id"])){ 
	...
	$fp = fopen($filename, 'a');
//記入された内容を書き出す
	fwrite($fp, $count."<>".$name."<>".$comment."<>".date('Y年m月d日 h時i分')."<>".$pass."\n");	
...
if(isset($_POST["delete"]) && !empty($_POST["delno"])){
	//ファイルから読み込んで配列に格納
	$filedel = file('mission2.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
	$delparts = explode("<>", $filedel[$num - 1]);
	if($delparts[4] == $passdel){
		//削除する行を配列から消去
...
if(isset($_POST["number"])){
	...
	foreach ($lines as $key => $value) {
	    $s = explode("<>", $value);
	    if ($s[0] == $num) {
	        $lines[$key] = $num . "<>" . $name . "<>" . $comment . "<>" . date('Y年m月d日 h時i分'). "<>" .$pass."\n";
	        break;
	    }
	}
	fclose($fp);

	file_put_contents('mission2.txt', $lines);
