...
<form method="post" action="mission_3-7_new.php">
<b>ユーザー登録</b><br /><br />
...
try{
	...
	if(!empty($_POST["usrname"]) && !empty($_POST["pass"])){ 
		...
		$sql = "INSERT INTO userlist(usrname,pass)VALUES(?,?)";
		$result = $pdo -> prepare($sql);
		$result ->execute(array($usrname,$pass));
	...
	//結果を出力
		...
		$sql = "SELECT * FROM userlist WHERE id=$count";
		$result = $pdo -> query($sql);
		
		echo $usrname."さんの登録が完了しました。<br />";
		foreach($result as $row){
			echo "ID:".$row['id'].'<br />';
			echo "NAME:".$row['usrname'].'<br />';
